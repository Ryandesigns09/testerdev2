...
passport.use(new TwitterStrategy({
    ...
    callbackURL: "http://www.eternalturtles.com/oauth/callback/twitter"
...
passport.deserializeUser(function(user, cb) {
  process.nextTick(function() {
    db.get('SELECT * FROM users WHERE id = ?', [ user.id ], function(err, row) {
      if (err) { return cb(err); }
      if (!row) { return cb(null, false); }
      return cb(null, { id: row.id, username: row.username, name: row.name, whitelist: row.whitelist, battlebadge: row.battlebadge, subject: row.subject });
    });
  });
});
